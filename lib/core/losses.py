...
def bin_idx_loss(batch):
    depth_gt = batch['depth_gt']
    has_depth = batch['has_smpl'].to(torch.bool)
    depthimg_idx_pred = batch['depth_out'][0]['predict_depth']

    batch_size = depthimg_idx_pred.size(0)
    device = depthimg_idx_pred.device
    if has_depth is not None:
        if torch.sum(has_depth.float()) > 0:
            depthimg_idx_pred = depthimg_idx_pred[has_depth] if depthimg_idx_pred is not None else depthimg_idx_pred
            depth_gt = depth_gt[has_depth] if depth_gt is not None else depth_gt
        else:
            return (torch.zeros(1).to(device), torch.zeros(1).to(device), torch.zeros(1).to(device), torch.zeros(1).to(device))
        
    foreground_mask = (depth_gt > -1)

    fore_depthimg_idx_pred = depthimg_idx_pred[foreground_mask]
    fore_depth_gt = depth_gt[foreground_mask]
    criterion_depth_idx_loss = torch.nn.SmoothL1Loss()
    fore_loss = criterion_depth_idx_loss(fore_depthimg_idx_pred, fore_depth_gt) 

    loss_depth = fore_loss
    return loss_depth
    
# def bin_bc_loss(batch):
#     depth_bc_gt = batch['depth_bc_gt']
#     has_depth = batch['has_smpl'].to(torch.bool)
#     depthimg_bc_pred = batch['depth_out'][0]['predict_bc']

#     batch_size = depthimg_bc_pred.size(0)
#     device = depthimg_bc_pred.device
#     if has_depth is not None:
#         if torch.sum(has_depth.float()) > 0:
#             depthimg_bc_pred = depthimg_bc_pred[has_depth] if depthimg_bc_pred is not None else depthimg_bc_pred
#             depth_bc_gt = depth_bc_gt[has_depth]
#         else:
#             return (torch.zeros(1).to(device), torch.zeros(1).to(device), torch.zeros(1).to(device), torch.zeros(1).to(device))
#     if depthimg_bc_pred is None:
#         loss_depth = None
#     else:
#         criterion_depth_bc_loss = torch.nn.BCEWithLogitsLoss()
#         loss_depth = criterion_depth_bc_loss(depthimg_bc_pred + 1e-10, depth_bc_gt)
        
#     return loss_depth
            
# collection = {'KPT2D': keypoint_loss, 'KPT3D': keypoint_3d_loss, 'SMPL':  smpl_losses,
#               'CAM_S': cam_depth_loss, 'CAM': cam_loss, 'V3D': vertice_loss, 'DEPTH': bin_idx_loss, 'DEPTHBC': bin_bc_loss }